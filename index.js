...
app.post("/validate", (req, res) => {
  //This is the route called by the login function
  knex
    .select("Email")
    .from("Users")
    .then((uname) => {
            ...
            .then((pass) => {
              if (pass[0].Password == req.body.password) {
                ...
                req.session.save(function (err) {
                  //this is what we were missing for a while when baking cookies
...
app.post("/editAccount", (req, res) => {
  ...
  knex
    ...
    .then((uname) => {
      ...
      if (
        !aUsernames.includes(req.body.email) ||
...
app.post("/submitResponses", (req, res) => {
  ...
  knex("ProductsOwned")
    ...
    .then(() => {
      ...
      return res.render("submitResponses", {
        name: name,
        id: id,
        question1: question1,
      });
